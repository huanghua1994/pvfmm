...
void fmm_test(int ker, size_t N, size_t M, Real_t b, int dist, int mult_order, int depth, MPI_Comm comm, const char *fname){
...
int main(int argc, char **argv){
  ...
  int    ker=       strtoul(commandline_option(argc, argv,  "-ker",     "1", false,
       "-ker  <int> =  (1)   : 1) Laplace potential\n\
                               ...
                               3) Stokes velocity\n\
                               4) Helmholtz"),NULL,10);
  const char *fname= commandline_option(argc, argv, "-inf", "xyz.csv", false, "-inf string = xyz.csv : input coord csv file name");
  commandline_option_end(argc, argv);
  pvfmm::Profile::Enable(true);

  // Run FMM with above options.
  pvfmm::Profile::Tic("FMM_Test",&comm,true);
  if(sp) fmm_test<float >(ker, N,M,b, dist,m,d,comm, fname);
  else   fmm_test<double>(ker, N,M,b, dist,m,d,comm, fname);
  pvfmm::Profile::Toc();
